    ...
    <div className="flex h-[35px] w-full items-center justify-between self-stretch">
      <div className="flex items-center gap-3">
        <span className="flex h-6 min-w-[28px] items-center justify-center gap-0.5 rounded-full bg-black px-1.5 text-xs font-semibold text-white">
          {isTop && <img src={ICONS.crown} alt="" className="h-3 w-3" aria-hidden="true" />}
          {String(rank).padStart(2, "0")}
        </span>

        <img src={ICONS.profileMember} alt={`${name} 프로필`} className="h-6 w-6 rounded-full" />
      ...
      <div className="flex items-center gap-4">
        <span className="flex items-center gap-1 text-sm text-gray-700">
          <img src={liked ? ICONS.heartActive : ICONS.heartInactive} alt="" className="h-4 w-4" aria-hidden="true" />
          {likeCount.toLocaleString()}
        </span>
        <button type="button" onClick={onViewWork} className="flex items-center gap-0.5 text-xs text-gray-900">
          작업물 보기
          <img src={ICONS.out} alt="" className="h-3.5 w-3.5" aria-hidden="true" />
        </button>
      </div>
    </div>
  );
